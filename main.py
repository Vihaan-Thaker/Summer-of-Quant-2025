...
def process_data(data):
    ...
    if len(data) >= 26:
        macd = ta.macd(data['close'], append=False)
...
def strat(data):
    ...
    for i in range(26, len(data)): # Starting from the 26th index to ensure MACD can be calculated

        # Check if there is a volume spike
        vol_spike=np.mean(data.loc[i -10: i, 'volume']) + 1.231*np.std(data.loc[i -10: i, 'volume'])

        # --- BUYING CONDITIONS ---
        buy_conditions = [
            data.loc[i, 'RSI'] < 70,
            data.loc[i, 'SMA_50'] > data.loc[i, 'SMA_200'],
            data.loc[i, 'MACD_Line'] > data.loc[i, 'Signal_Line'],
        ]
        ...
        if position == 0:
            if data.loc[i, 'volume'] > vol_spike:
                if sum(buy_conditions) >= 2:
                    ...
                    trailing_loss = data.loc[i, 'close'] - (data.loc[i, "ATR"] * trailing_stop_multiplier)

                elif sum(sell_conditions) >= 2:
                    data.loc[i, 'signals'] = -1  # Short signal
                    position = -1
            ...
            if trend_rev: # Trend reversal detected
                ...
                position = -1
                trailing_loss = data.loc[i,'close'] + (data.iloc[i]["ATR"] * trailing_stop_multiplier)
                num_wrong=0
                ...
                if data.iloc[i]["close"] < trailing_loss:
                    data.loc[i, 'signals'] = -1
                    position = 0
                    data.loc[i, 'trade_type'] = 'CLOSE'
                else: # Update the trailing stop
                    trailing_loss = max(trailing_loss, data.iloc[i]["close"] - (data.iloc[i]["ATR"] * trailing_stop_multiplier))   
            ...
            if trend_rev: # Trend reversal detected
                ...
                if data.iloc[i]["close"] > trailing_loss:
                    ...
                    trailing_loss = min(trailing_loss, data.iloc[i]["close"] + (data.iloc[i]["ATR"] * trailing_stop_multiplier))
    return data

def main():
    data = pd.read_csv("/Users/vihaa/Downloads/Project_files/BTC_2019_2023_1d.csv")
    processed_data = process_data(data) # process the data
    result_data = strat(processed_data) # Apply the strategy
    csv_file_path = "/Users/vihaa/Downloads/Project_files/final_data.csv" 
    result_data.to_csv(csv_file_path, index=False)

    bt = BackTester("BTC", signal_data_path="/Users/vihaa/Downloads/Project_files/final_data.csv", master_file_path="/Users/vihaa/Downloads/Project_files/final_data.csv", compound_flag=1)
    bt.get_trades(1000)
